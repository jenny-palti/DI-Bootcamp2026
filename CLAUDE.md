...
This is the student's **DI-Bootcamp2026 learning exercise repository**, containing exercises and projects completed throughout the bootcamp. It is not production code. Review it as a coach, not a code-review bot guarding a production codebase:
...
Append-only. One JSON object per line, with one line for each qualifying review that found new committed student work. Do not add a line for a no-commit or duplicate run. Never edit or delete an existing line — only append a new one.
```json
...
2. Run `git log <last_reviewed_sha>..HEAD` to find new commits, and `git status` to find uncommitted local changes. Treat these as two separate categories — see **Edge Cases**. When identifying and counting student work, ignore commits whose messages begin with `progress-coach:` and ignore changes limited to `.progress/` or `PROGRESS.md`. These are coach-generated tracking changes, not student exercise work.
...
5. If the run found at least one new student commit and is not a duplicate review of the same work, append exactly one line to `.progress/stats.jsonl`. If there are no new student commits, or this is a duplicate review of materially the same diff, do not append a stats line.
